...
# ─── Path Setup ───────────────────────────────────────────────────────────────
# Ensure backend/ modules can be imported directly (block_scheduler, etc.)
...
if _db_env:
    _db_p = Path(_db_env)
    DB_PATH = _db_p if _db_p.is_absolute() else (BASE_DIR / _db_p)
else:
...
try:
    from backend.block_scheduler import ensure_db_tables
    ensure_db_tables()
...
async def get_railway_image():
    img_path = ASSETS_DIR / "railway.jpg"
    if img_path.exists():
...
async def serve_map():
    return FileResponse(str(FRONTEND_DIR / "map.html"), headers={"Cache-Control": "no-cache, no-store, must-revalidate"})


@app.get("/block", response_class=FileResponse)
@app.get("/block.html", response_class=FileResponse)
async def serve_block():
    return FileResponse(str(FRONTEND_DIR / "block.html"), headers={"Cache-Control": "no-cache, no-store, must-revalidate"})
...
async def serve_degradation():
    return FileResponse(str(FRONTEND_DIR / "degradation.html"), headers={"Cache-Control": "no-cache, no-store, must-revalidate"})
...
async def get_network():
    net_file = DATA_DIR / "corridor_network.json"
    if net_file.exists():
...
async def get_stations():
    stations_file = DATA_DIR / "all_stations.json"
...
async def api_simulated_trains_dataset():
    ...
    dataset_path = DATA_DIR / "simulated_trains.json"
...
async def api_train_position(train: str, corridor_id: Optional[str] = None):
    ...
    if len(stops) >= 2:
        try:
            from backend.train_position_calculator import calculate_train_position_from_stops
...
async def api_fetch_trains(background_tasks: BackgroundTasks,
    ...
    def _run_fetch():
        try:
            ...
            args = [sys.executable, str(BACKEND_DIR / "fetch_trains.py")]
            if corridor_id:
...
try:
    import backend.block_scheduler as _bs
except ImportError:
...
from backend.dbscan_clustering import run_dbscan
...
from backend.nsga2_optimizer import run_nsga2
from backend.gap_calculator import load_trains_from_db
...
def get_timetable_gaps():
    ...
    gap_file = DATA_DIR / "gap_schedules.json"
    if not gap_file.exists():
        try:
            from backend.gap_calculator import calculate_all_gap_schedules
